...
from knowledge_base.api_key_manager import get_api_key
...
class AIService:
    ...
    def get_api_key(self, key_name: str) -> str:
        ...
        if not api_key:
            print(f"警告：未找到API Key '{key_name}'，请使用 set_api_key.py 脚本设置")
        return api_key
    ...
    def _get_default_prompt_template(self) -> str:
        ...
        db = next(get_db())
        try:
            default_template = db.query(PromptTemplate).filter(PromptTemplate.is_default == 1).first()
            if default_template:
                return default_template.template_content
            else:
                # 如果没有默认模板，返回一个默认的提示词
                return "你是一个专业的淘宝电商客服，需要：\n1. 保持友好、专业的语气\n2. 快速理解用户问题\n3. 提供准确的解决方案\n4. 遇到不确定的问题，及时请求用户提供更多信息\n\n用户问题：{user_question}"
        finally:
            db.close()
    ...
    async def generate_response(self, user_message: str, conversation_history: list = None) -> str:
        ...
        # 替换模板中的用户问题
        prompt = prompt_template.replace("{user_question}", user_message)
        
        # 这里可以添加真正的大模型调用代码
        # 目前使用模拟响应
        print(f"使用Prompt模板: {prompt}")
